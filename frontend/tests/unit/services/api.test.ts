  ...
  describe("Authenticated Requests", () => {
    ...
    it("submitReview calls correct endpoint", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: "POST",
          body: JSON.stringify({ cardId: "card-123", rating: "good" }),
        }),
      );
    });
    ...
    it("getDeck calls correct endpoint", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining("/decks/deck-1"),
        expect.anything(),
      );
    });
    ...
    it("getGeneralReviewSession calls correct endpoint", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining("/review/session/general"),
        expect.anything(),
      );
    });
    ...
    it("getDeckReviewSession calls correct endpoint", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining("/review/deck/deck-1"),
        expect.anything(),
      );
    });
  });
