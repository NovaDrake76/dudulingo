    ...
    const startSession = async () => {
      ...
      try {
        ...
        if (sessionData.cards && sessionData.cards.length > 0) {
          ...
          Alert.alert(
            "All done!",
            "You have no cards to review at the moment.",
            [{ text: "OK", onPress: () => router.back() }],
          );
        }
      } catch (error) {
        console.error("Failed to start session:", error);
        Alert.alert("Error", "Could not start the review session.", [
          { text: "OK", onPress: () => router.back() },
        ]);
      } finally {
        setLoading(false);
  ...
  const renderQuestionContent = () => {
    ...
    return (
      <View style={styles.questionContentContainer}>
        ...
        {imageUrl && !questionType.includes("word_multiple_choice_image") && (
          <Image source={{ uri: imageUrl }} style={styles.questionImage} />
        )}
        {word && <Text style={styles.questionWord}>{word}</Text>}
        {prompt && !questionType.includes("image_word_multiple_choice") && (
          <Text style={styles.questionTranslation}>{prompt}</Text>
        )}
      </View>
    );
  };
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.progressText}>
          Card {currentQuestionIndex + 1} of {sessionCards.length}
        </Text>
      </View>
        ...
        {questionType?.includes("multiple_choice") && options && (
          <View style={styles.optionsContainer}>
            {options.map((option, index) => {
              const isImageOption =
                typeof option === "object" && "imageUrl" in option;
              const optionText = isImageOption
                ? option.text
                : (option as string);
