...
async function getLearningCards(
  userId: string,
  limit: number,
  excludedIds: string[],
) {
...
router.get("/session/general", async (req: any, res) => {
  try {
    ...
    if (sessionProgress.length < sessionSize) {
      const learningProgress = await getLearningCards(
        userId,
        sessionSize - sessionProgress.length,
        Array.from(seenCardIds),
      );
    ...
    if (sessionProgress.length < sessionSize) {
      const newProgress = await getNewCards(
        userId,
        sessionSize - sessionProgress.length,
        Array.from(seenCardIds),
      );
    ...
    const sessionQuestions = await Promise.all(
      sessionProgress.map((progress) =>
        createQuestionData(progress.cardId, progress),
      ),
    );
